...
export function WorkspaceTabMenu({ children }: { children: React.ReactElement }) {
  ...
  return (
    <SessionTabMenu onHideTabBar={hideTabBar} storedSessionId={selected}>
      {children}
    </SessionTabMenu>
  )
}
...
export const watchSessionTiles = paneMirror<SessionTile>({
  ...
  tabWrap: (storedSessionId, tab) => (
    <SessionTabMenu onClose={() => requestCloseSessionTile(storedSessionId)} storedSessionId={storedSessionId}>
      {tab}
    </SessionTabMenu>
  ),
